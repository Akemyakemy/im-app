...
一款基于 C++ 高性能后端与 Vue3 + Electron 跨平台前端构建的即时通讯应用，支持实时消息收发、用户在线状态同步等核心功能。全程适配 WSL2+Ubuntu 开发环境，采用 Docker 简化部署流程，技术栈选型兼顾性能与跨平台兼容性。
...
im_app/
├── im_server/          # 后端服务 (C++ / CMake 构建)
│   ├── db/             # 数据库初始化脚本 (MySQL + Redis)
│   ├── config.json     # 核心配置文件 (JSON 格式，配置数据库/端口/Socket等)
...
后端采用 CMake 构建，支持两种部署方式：Docker 容器化（推荐，无需手动配置依赖）、本地原生编译（适合调试）。
...
2. 构建并启动服务（自动拉起 MySQL、Redis 依赖服务）
       `docker-compose up --build -d`

3. 启动成功验证
       服务启动后，各组件端口映射（宿主机可访问）：

    - IM 服务 Socket 端口：**9090**（核心，实时消息传输）

    - IM 服务辅助 API 端口：**8080**（用户注册、登录等接口）
...
libssl-dev libjsoncpp-dev libuuid-dev zlib1g-dev
# 验证依赖版本 (需满足：g++ 支持 C++17+，CMake ≥ 3.16)
g++ --version
cmake --version`
...
前端基于 Vue3 + Electron 构建，集成 Socket 客户端实现实时消息交互，提供一键配置脚本简化操作。
...
4. 运行效果
    ...
    - Electron 窗口：前端应用主界面，自动连接后端 Socket (ws://localhost:9090)
...
||Socket|前端与后端双向实时通信，支撑即时消息核心功能|
|后端技术|C++|高性能后端开发语言，保障服务并发处理能力|
||CMake|跨平台构建工具，简化 C++ 项目编译与依赖管理|
||MySQL|关系型数据库，持久化存储用户信息、会话记录、聊天历史等核心数据|
||Redis|缓存数据库，提升用户在线状态、临时消息等场景的响应速度|
||JSON|轻量数据交换格式，用于后端配置文件与前后端数据传输|
|部署工具|Docker|容器化打包服务与依赖，实现环境隔离、一键部署|
## 📝 开发指南

### 1. VSCode 开发配置

- **后端调试配置**：在 im_server 目录下创建 .vscode/launch.json，配置 CMake 调试参数，支持断点调试、变量监控。
...
- 提交信息格式：`[模块] 操作描述`，示例：
    ...
    - [im_server] 修复 Socket 断连重连异常问题

    - [im_client] 新增聊天页面表情发送功能

    - [docs] 更新 README 环境配置步骤

### 4. 常见问题排查

- **Docker 启动失败**：检查 WSL2 虚拟化是否开启，执行 `sudo systemctl status docker` 查看服务状态，修复权限或依赖问题。

- **前端 Socket 连接失败**：确认后端服务已启动，9090 端口未被占用（执行 `netstat -an | grep 9090` 检查），WSL2 端口映射正常。

- **后端本地编译失败**：核对依赖库是否安装完整，C++ 编译器是否支持 C++17+，CMake 版本是否达标。

- **数据库连接异常**：检查 MySQL/Redis 服务是否启动，配置文件中的连接地址、端口、用户名密码是否正确。
